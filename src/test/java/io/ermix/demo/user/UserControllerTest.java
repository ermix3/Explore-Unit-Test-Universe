...
class UserControllerTest {
  ...
  void shouldCreateNewUser() {
    ...
    ResponseEntity<UserResponse> response =
        restTemplate.postForEntity(baseUrl, userRequest, UserResponse.class);
  ...
  void shouldFindAllUsers() {
    ...
    UserResponse[] users =
        restTemplate.getForObject(baseUrl, UserResponse[].class);
  ...
  void shouldThrowNotFoundWhenInvalidUserID() {
    ResponseEntity<ErrorResponse> response = restTemplate.getForEntity(
        baseUrl + "/" + new Random().nextLong(), ErrorResponse.class);
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
    assertThat(response.getBody()).isNotNull();
  ...
  void patch() {
    ...
    UserRequest userRequest = buildUserRequest();
    //        UserResponse patchUser = restTemplate.patchForObject(baseUrl + "/"
    //        + savedUser.getId(),  new HttpEntity<>(userRequest),
    //        UserResponse.class);
    ResponseEntity<UserResponse> response = restTemplate.exchange(
        baseUrl + "/" + savedUser.getId(), HttpMethod.PATCH,
        new HttpEntity<>(userRequest), UserResponse.class);
  ...
  void shouldThrowNotFoundWhenDeleteWithInvalidID() {
    ResponseEntity<ErrorResponse> response =
        restTemplate.exchange(baseUrl + "/" + new Random().nextLong(),
  ...
  @Test
  @DisplayName(
      "Should return 400 Bad Request when creating user with invalid data")
  void
  shouldReturnBadRequestWhenCreatingUserWithInvalidData() {
    UserRequest invalidUserRequest =
        new UserRequest("", "invalid-email", "123", null);

    HttpHeaders headers = new HttpHeaders();
    headers.setContentType(MediaType.APPLICATION_JSON);

    HttpEntity<UserRequest> requestEntity =
        new HttpEntity<>(invalidUserRequest, headers);

    ResponseEntity<ErrorResponse> response =
        restTemplate.postForEntity(baseUrl, requestEntity, ErrorResponse.class);
    ...
    assertThat(errorResponse.details())
        .contains("Username is mandatory", "Email should be valid");
  }

  @Test
  @DisplayName(
      "Should return 400 Bad Request when updating user with invalid data")
  void
  shouldReturnBadRequestWhenUpdatingUserWithInvalidData() {
    User savedUser = getUser();
    ...
    ResponseEntity<ErrorResponse> response =
        restTemplate.exchange(baseUrl + "/" + savedUser.getId(), HttpMethod.PUT,
